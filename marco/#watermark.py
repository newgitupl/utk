...
# Updated color options with all emojis
COLORS = {
    "White": "🤍",
    "Black": "🖤", 
    "Red": "❤️",
...
async def watermark_settings(_, callback_query: CallbackQuery):
    ...
    text = (
        ...
        f"🎨 Text Colour: {current_color.title()} {COLORS.get(current_color.title(), '')}\n"
...
async def change_watermark_color(client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    settings = get_watermark_settings(user_id)
    current_color = settings.get('color', 'white').title()
    ...
    buttons.append([InlineKeyboardButton("↩️ Back", callback_data="watermark_marco")])
    
    await callback_query.message.edit_media(
        media=InputMediaPhoto(
            media=SETTINGS_IMAGE,
            caption=f"**Current Color:** {current_color} {COLORS.get(current_color, '')}\n\nChoose a color:"
        ),
...
def generate_preview_image(text: str, font_path: str, color_name: str):
    ...
    color_hex = COLOR_HEX.get(color_name.lower(), "#000000")  # Default to black
    ...
    try:
        await callback_query.message.edit_media(
            media=InputMediaPhoto(
                ...
                f"Text: {text}\n"
                f"Font: {font.split('.')[0]}\n"
                f"Color: {color.title()} {COLORS.get(color.title(), '')}\n\n"
...
@Client.on_callback_query(filters.regex("^set_wm_color_"))
async def set_watermark_color(client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    color = callback_query.data.split("_")[-1]  # e.g., "red"
    
    update_watermark_settings(user_id, {"color": color})
    emoji = COLORS.get(color.title(), "")
    await callback_query.answer(f"Color set to {color.title()} {emoji}!")
    await watermark_settings(client, callback_query)
